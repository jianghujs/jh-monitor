...
if os.path.exists('/www/server/jh-monitor'):
...
try:
    ...
    jh.execShell("pip install pyopenssl")
    import OpenSSL
...
class cert_api:
    ...
    def __init__(self):
        self.__user_agent = 'JH-Panel:' + jh.getRandomString(8)
        self.__save_path = jh.getServerDir() + '/web_conf/letsencrypt'
    ...
    def readConfig(self):
        if not os.path.exists(self.__cfg_file):
            ...
            self.__config['apis'] = {}
            self.__config['email'] = jh.M('users').where(
                'id=?', (1,)).getField('email')
            if self.__config['email'] in ['midoks@163.com']:
                self.__config['email'] = None
            self.saveConfig()
            return self.__config
        tmp_config = jh.readFile(self.__cfg_file)
        if not tmp_config:
            return self.__config
        try:
    ...
    def createCertCron(self):
        ...
        try:
            ...
            echo = jh.md5(jh.md5('panel_renew_lets_cron'))
            cron_id = jh.M('crontab').where('echo=?', (echo,)).getField('id')

            cron_path = jh.getServerDir() + '/cron'
            if not os.path.exists(cron_path):
                jh.execShell('mkdir -p ' + cron_path)
...
''' % (jh.getRunDir(), logs_file)
            ...
            if type(cron_id) != int:
                ...
                insert_id = jh.M('crontab').add('name,type,where1,where_hour,where_minute,echo,addtime,status,save,backup_to,stype,sname,sbody,urladdress', (
                ...
                jh.writeFile(file, cmd)
                jh.execShell('chmod 750 ' + file)
                jh.M('crontab').where('id=?', (cron_id)).save('sbody', (cmd,))
        except Exception as e:
            print(jh.getTracebackInfo())
    ...
    def getSiteRunPathByid(self, site_id):
        if jh.M('sites').where('id=?', (site_id,)).count() >= 1:
            site_path = jh.M('sites').where('id=?', site_id).getField('path')
            if not site_path:
                return None
    ...
    def getSiteRunPath(self, domains):
        site_id = 0
        for domain in domains:
            site_id = jh.M('domain').where("name=?", domain).getField('pid')
            if site_id:
                break
    ...
    def clearAuthFile(self, index):
        ...
        if os.path.exists(acme_path):
            jh.execShell("rm -f {}/*".format(acme_path))

        acme_path = jh.getServerDir() + '/stop/.well-known/acme-challenge'
        if os.path.exists(acme_path):
            jh.execShell("rm -f {}/*".format(acme_path))
    ...
    def writeAuthFile(self, auth_to, token, acme_keyauthorization):
        try:
            ...
            if not os.path.exists(acme_path):
                ...
                jh.setOwn(acme_path, 'www')
            wellknown_path = '{}/{}'.format(acme_path, token)
            jh.writeFile(wellknown_path, acme_keyauthorization)
            jh.setOwn(wellknown_path, 'www')
    ...
    def saveOrder(self, order_object, index):
        ...
        renew = False
        if not index:
            index = jh.md5(json.dumps(order_object['identifiers']))
        else:
            renew = True
    ...
    def dumpDer(self, cert_path):
        cert = OpenSSL.crypto.load_certificate(
            OpenSSL.crypto.FILETYPE_PEM, jh.readFile(cert_path + '/cert.csr'))
    ...
    def subAllCert(self, key_file, pem_file):
        ...
        paths = ['/www/server/jh-monitor/data/letsencrypt']
        is_panel = False
        for path in paths:
            ...
            for p_name in os.listdir(path):
                ...
                # 替换新的证书文件和基本信息
                jh.writeFile(to_pem_file, jh.readFile(pem_file, 'rb'), 'wb')
                jh.writeFile(to_key_file, jh.readFile(key_file, 'rb'), 'wb')
                jh.writeFile(to_info, json.dumps(cert_init))
    ...
    def saveCert(self, cert, index):
        try:
            ...
            jh.writeFile(key_file, cert['private_key'])
            jh.writeFile(pem_file, cert['cert'] + cert['root'])
            jh.writeFile(path + "/cert.csr", cert['cert'])
            jh.writeFile(path + "/root_cert.csr", cert['root'])

            # 转为IIS证书
            pfx_buffer = self.dumpPkcs12(
                cert['private_key'], cert['cert'] + cert['root'], cert['root'], domain_name)
            jh.writeFile(path + "/fullchain.pfx", pfx_buffer, 'wb+')
...
'''
            jh.writeFile(path + '/readme.txt', ps)
            self.subAllCert(key_file, pem_file)
        except:
            writeLog(jh.getTracebackInfo())
    ...
    def applyCert(self, domains, auth_type='http', auth_to='Dns_com|None|None', args={}):
        ...
        try:
            ...
            if os.path.exists(auth_to):
                jh.execShell("rm -rf {}/.well-known".format(auth_to))
            return cert
        except Exception as ex:
            ex = str(ex)
            if ex.find(">>>>") != -1:
    ...
    def extractZone(self, domain_name):
        top_domain_list = ['.ac.cn', '.ah.cn', '.bj.cn', '.com.cn', '.cq.cn', '.fj.cn', '.gd.cn', '.gov.cn', '.gs.cn',
                           ...
                           '.com.mo', '.com.mp', '.com.mq', '.com.mr', '.com.ms', '.com.mt', '.com.mv', '.com.jh', '.com.mx',
    ...
    def renewCertOther(self):
        cert_path = "{}/vhost/cert".format(jh.getRunDir())
        ...
        for siteName in os.listdir(cert_path):
            try:
                ...
                if not os.path.exists(cert_file):
                    continue  # 无证书文件
                siteInfo = jh.M('sites').where('name=?', siteName).find()
                if not siteInfo:
                    continue  # 无网站信息
    ...
    def getHostConf(self, siteName):
        return jh.getServerDir() + '/web_conf/nginx/vhost/' + siteName + '.conf'
    ...
    def renewCertTo(self, domains, auth_type, auth_to, index=None):
        ...
        if os.path.exists(auth_to):
            if jh.M('sites').where('path=?', auth_to).count() == 1:
        ...
        try:
            ...
            # 发送通知
            notify_msg = jh.generateCommonNotifyMessage("|-域名[{}]SSL证书续签完成！".format(str(domains)))
            jh.notifyMessage(msg=notify_msg, title="续签成功提醒", stype='续签完成', trigger_time=0)
        except Exception as e:
            if str(e).find('429') > -1:
                msg = '至少超过7天，才能续期!'
                writeLog("|-" + msg)
            ...
            # 发送通知
            notify_msg = jh.generateCommonNotifyMessage("|-续签[{}]SSL证书异常: {}".format(str(domains), msg) + '\n请注意!')
            jh.notifyMessage(msg=notify_msg, title="续签失败提醒", stype='续签失败', trigger_time=0)
            ...
            if index:
                ...
                for i in self.__config['orders'].keys():
                    ...
                    for domain in self.__config['orders'][i]['domains']:
                      ...
                      site = None
                      try:
                        current_domain = jh.M('domain').where("name=?", (domain,)).field('pid,name').find()
                        if current_domain:
                          site = jh.M('sites').where("id=?", (current_domain['pid'],)).field('name,path,status').find()
                    ...
                    if self.__config['orders'][i]['auth_to'].find('|') == -1 and self.__config['orders'][i]['auth_to'].find('/') != -1:
                        if not os.path.exists(self.__config['orders'][i]['auth_to']):
                            ...
                            for domain in self.__config['orders'][i]['domains']:
                                ...
                                # 域名不存在？
                                current_domain = jh.M('domain').where("name=?", (domain,)).field('pid,name').find()
                                current_binding = jh.M('binding').where("domain=?", domain).field('pid,domain,path').find()
                                print("####current_domain", current_domain)
                                print("####current_binding", current_binding)
                                if not jh.M('domain').where("name=?", (domain,)).count() and not jh.M('binding').where("domain=?", domain).count():
                                    auth_to = None
                                    writeLog(
                    ...
                    cent_valid = False
                    for domain in self.__config['orders'][i]['domains']:
                        ssl_path = jh.getWebConfSSLDir() + '/' + domain
    ...
    def revokeOrder(self, index):
        ...
        if res.status in [200, 201]:
            if os.path.exists(cert_path):
                jh.execShell("rm -rf {}".format(cert_path))
            del(self.__config['orders'][index])
            self.saveConfig()
            return jh.returnJson(True, "证书吊销成功!")
        return res.json()
    ...
    def do(self, args):
        ...
        try:
            ...
            writeLog("|-{}".format(jh.getTracebackInfo()))
            exit()
        ...
        writeLog("证书到期时间: {}".format(
            jh.formatDate(times=cert['cert_timeout'])))
...
// renew
cd /www/server/jh-monitor && python3 class/core/cert_api.py --renew=1 --index=46c558aa1fae96facf36ac7df8eb4750
// revoke
cd /www/server/jh-monitor && python3 class/core/cert_api.py --revoke=1 --index=370423ed29481b2caf22e36d90a6894a



python3 class/core/cert_request.py --domain=dev38.cachecha.com --type=http --path=/Users/midoks/Desktop/jhdev/wwwroot/test

python3 class/core/cert_api.py --domain=dev38.cachecha.com --type=http --path=/Users/midoks/Desktop/jhdev/wwwroot/test
