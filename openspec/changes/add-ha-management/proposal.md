...
- 新增江湖云监控“主备管理”模块，提供由插件注册上来的主备关系列表、主备组详情多标签页、手动切换、告警提醒和日志查看；云监控页面不提供手动添加主备关系入口。
...
- 云监控地址和主备关系名称作为插件独立配置项，默认地址留空；留空时插件只做本机能力，不注册关系、不上传状态和日志。
...
- `ha-management-ui`: Cloud monitor SHALL provide a logged-in HA management UI for plugin-registered HA pairs, table list, and multi-tab group detail views.
...
- APIs: new logged-in HA management APIs for listing/detailing HA pairs and operating switch runs, plus signed public plugin APIs for registering pairs, pulling desired state, reporting state, reporting switch events, acknowledging phases, and callback configuration.
- Data: new HA pair/state/switch-run/callback records in SQLite; switch logs stored as files, not ES documents.
- Operations: two panel servers both install `ha_manager`; operators bind peer SSH in the plugin, configure HA pair name and cloud monitor URL in the plugin, and perform manual switch from either cloud monitor or the plugin homepage.
