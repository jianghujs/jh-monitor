...
UNINSTALL_MySQL()
{
    ...
    if [ "$MYSQLD_CHECK" != "" ];then
        ...
        if [ "$yes" != "yes" ];then
            ...
            cd /www/server/jh-monitor/plugins/mysql && sh install.sh uninstall 8.0
            echo "卸载MySQL成功!"
        fi
    fi
}
...
UNINSTALL_OP()
{
    if [ -f /www/server/openresty ];then
        ...
        if [ "$yes" != "yes" ];then
            ...
            cd /www/server/jh-monitor/plugins/openresty && sh install.sh uninstall
            echo "卸载OpenResty成功!"
        fi
    fi
}
        ...
        if [ "$yes" != "yes" ];then
            ...
            for PHP_VER in ${PHP_VER_LIST[@]}; do
                if [ -d /www/server/php/${PHP_VER} ];then
                    cd /www/server/jh-monitor/plugins/php && bash install.sh uninstall ${PHP_VER}
                fi
...
UNINSTALL_MEMCACHED()
{
    if [ -d /www/server/memcached ];then
        ...
        if [ "$yes" != "yes" ];then
            ...
            cd /www/server/jh-monitor/plugins/memcached && bash install.sh uninstall
            echo "卸载Memcached成功"
        fi
    fi
}
...
UNINSTALL_REDIS()
{
    if [ -d /www/server/redis ];then
        ...
        if [ "$yes" != "yes" ];then
            ...
            cd /www/server/jh-monitor/plugins/redis && bash install.sh uninstall 7.0.4
            echo "卸载Redis成功"
        fi
    fi
}
