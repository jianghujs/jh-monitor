...
if [ -d "/www/server/jh-monitor" ]; then
  echo "目录 /www/server/jh-monitor 存在"
...
# git clone jh-monitor from gitee (cn only)
echo "git clone https://gitee.com/jianghujs/jh-monitor /www/server/jh-monitor"
git clone https://gitee.com/jianghujs/jh-monitor /www/server/jh-monitor
...
if [ "$VERSION_ID" == "9" ];then
	sed "s/flask==2.0.3/flask==1.1.1/g" -i /www/server/jh-monitor/requirements.txt
	sed "s/cryptography==3.3.2/cryptography==2.5/g" -i /www/server/jh-monitor/requirements.txt
	sed "s/configparser==5.2.0/configparser==4.0.2/g" -i /www/server/jh-monitor/requirements.txt
	sed "s/flask-socketio==5.2.0/flask-socketio==4.2.0/g" -i /www/server/jh-monitor/requirements.txt
	sed "s/python-engineio==4.3.2/python-engineio==3.9.0/g" -i /www/server/jh-monitor/requirements.txt
fi
...
# 安装后文件会被清空(cn only)
echo "True" > /www/server/jh-monitor/data/net_env_cn.pl
